...
const files = [
    {
        ...
        size: '3.9 MB',
        source:
            '/images/witches-romance-poster.jpg',
    },
    {
        id: 2,
        title: 'Film 2',
...
const InternationalDramas = () => {
    const router = useRouter()
    const [dataList, setDataList] = useState<Movie[]>([])
    const [internationalDramas, setInternationalDramas] = useState<Movie[]>([])
    const fetchData = () => {
    ...
    useEffect(() => {
        const filteredMovies = dataList.filter(data => data.videoCategory === 'International Dramas')
        setInternationalDramas(filteredMovies)
    }, [dataList]);
...
export default InternationalDramas;