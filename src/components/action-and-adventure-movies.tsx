...
const ActionAndAdventureMovies = () => {
    ...
    useEffect(() => {
        const filteredMovies = dataList.filter(data => data.videoCategory === 'Action & Adventure Movies')
        setActionAndAdventureMovies(filteredMovies)
    }, [dataList]);
    ...
    return (
        <div className="mx-20 overflow-x-auto">
            <div role="list"
                ...
                {actionAndAdventureMovies.map((movie) => (
                    <div key={movie.movieId} className="relative scrollbar-item shadow shadow-white rounded-lg">
                        ...
                        <div className="flex flex-row justify-between items-center px-2 bg-neutral-900 rounded">
                            <div>
                                <p className="pointer-events-none mt-2 truncate text-sm font-medium text-gray-300 movie-name-length">{movie.movieName}</p>
